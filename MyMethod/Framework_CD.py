...
class Courier:
    def __init__(self, num: int, location, schedule, weight, station_num, preferences, quality):
        """
...
def GenerateOriginSchedule(fw_station_set, preference):
    ...
    for station in fw_station_set:
        ...
        for task in station.station_task_set:
            ...
            courier = Courier(num, station.l_node, task_schedule_set, round(
                sum_task_weight, 2), station.num, preference)
...
def WalkAlongRoute(courier, time, c_location, a_lengt, time_cost, real_time, courier_set1, station_set):
    ...
    if len(courier.re_schedule) != 0:
        ...
        if (VELOCITY * 1000) * time >= lengt:
            ...
            WalkAlongRoute(courier, time, courier.location, lengt,
                           time_cost, real_time, courier_set1, station_set)
        else:
...
def Greedy(station_set, f_courier_set, ff_pick_task_set, time_window, u, realtime):
    ...
    for i1 in range(int(14400 / realtime)):
                    ...
                    if len(can_do_courier_set) == 0:
                        ...
                        if len(can_do_courier_set) != 1:
                            ...
                            for c in can_do_courier_set:
                                result_pre, result_bid, result_extime = FBP_GA(
                                    c, t, u, time_count)
                                if result_pre == "a":
                                    pass_num += 1
                                    if pass_num % 3 == 0:
                            ...
                            if result_pre == "a":
                                ...
                                insert_index = best_courier.re_schedule.index(
                                    f_result_pre)
                                best_courier.re_schedule.insert(
                                    insert_index + 1, t)
        ...
        sys.stdout.write("\r rate of Greedy progress: %0.2f %%, %d" %
                         (percent, sucess_num))
        sys.stdout.flush()
        time.sleep(0.0001)
        ...
        for courier in f_courier_set:
            WalkAlongRoute(courier, realtime, courier.location,
                           0, 0, time_count, f_courier_set, station_set)
    endtime = time.time()
    avg_time = Decimal((sum_time_count / len(ff_pick_task_set))
                       * 1000).quantize(Decimal('0.00'))
    avg_time1 = Decimal((sum_time_count / sucess_num) *
                        1000).quantize(Decimal('0.00'))
    sum_time1 = Decimal(sum_time_count * 1000).quantize(Decimal('0.00'))
    sum_time2 = Decimal(sum_time_count / (14400 / time_window)
                        * 1000).quantize(Decimal('0.00'))
    sucess_rate = Decimal((sucess_num / len(ff_pick_task_set))
    ...
    each_bidding = Decimal((sum_bidding / sucess_num)
...
if __name__ == "__main__":
    print("Divide执行了")
    generate_station = GenerateStation(
        "Data/map_ChengDu", "Data/order_20161101_deal", 11)
    print("finish")
    ...
    for r in rounds:
        ...
        fw_f_courier_set1 = GenerateOriginSchedule(
            fw_station_set1, parameter_bidding_w)
        ...
        if p == 0.01:
            ...
            Greedy(fw_station_set1, fw_f_courier_set1,
                   fw_ff_pick_task_set1, time_window, u, realtime)
