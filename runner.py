...
def _add_common_environment_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--rl-lr-actor", type=float, default=2e-4, help="Actor learning rate for RL-CAPA. Lower than critic by default to keep the policy from outpacing critic estimates.")
    parser.add_argument("--rl-lr-critic", type=float, default=1e-3, help="Critic learning rate for RL-CAPA. Higher than actor to keep value targets ahead of policy updates.")
...
def build_algorithm_kwargs(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.algorithm in {"basegta", "capa", "greedy", "mra", "ramcom"}:
